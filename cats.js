...
// Создаём родительский блок, в котором будут расположены другие блоки с информацией о котах
let catsDiv = document.createElement("div");
catsDiv.classList.add(".cats");
document.body.appendChild(catsDiv);

// Проходимся по массиву и создаём анкету для каждого кота
for(let cat of cats) {
    // Внутри блока cats создаём блок(parentDiv), в котором будут ещё 2 блока(imageDiv, captionDiv)
    // В imageDiv будет изображение кота, в captionDiv текст, прикреплённый к изображению
    // Все эти блоки нужны для того, чтобы сделать из них флексбоксы и выровнять по красоте
    ...
    parentDiv.setAttribute("id", cat.id);
    
    catsDiv.appendChild(parentDiv);
    ...
    p.innerHTML = `
        ...
        <br><br>Стоимость: $${cat.rate}
    `

    // Стили элементов
    catsDiv.style.display = "flex";
    catsDiv.style.flexDirection = "column";
    catsDiv.style.alignItems = "center";
    catsDiv.style.gap = "50px";
    catsDiv.style.marginTop = "50px";
    catsDiv.style.marginBottom = "50px";
    
    parentDiv.style.display = "flex";
    parentDiv.style.flexDirection = "row";
    parentDiv.style.alignItems = "center";
    parentDiv.style.gap = "50px";
